...
impl super::StateInner {
    fn continue_auth(&mut self, id: usize, ctx: CommandContext<'_>) -> Result {
        ...
        let decoded = match client.auth_buffer_decode() {
            ...
            Err(err) => {
                ...
                ctx.rb
                    .reply(rpl::ERR_SASLFAIL, 2 + lines::SASL_FAILED.len(), |msg| {
        ...
        match self
            .auth_provider
            .next_challenge(id, &decoded, &mut challenge)
        {
            Ok(Some(user)) => {
                log::debug!("{}:     now authenticated", ctx.id);

                let capacity =
                    2 + 2 * client.nick().len() + 1 + client.full_name().len() + 1 + user.len();
                ...
                ctx.rb
                    .reply(rpl::SASLSUCCESS, 2 + lines::SASL_SUCCESSFUL.len(), |msg| {
                        msg.trailing_param(lines::SASL_SUCCESSFUL);
                    });

                if client.capabilities().account_notify {
                    ctx.rb
                        .message(client.full_name(), "ACCOUNT", 1 + user.len(), |msg| {
                            msg.param(&user);
                        });
                }
                ...
                account_notify
                    .message(client.full_name(), "ACCOUNT")
                    .param(&user);
    ...
    pub fn cmd_authenticate(&mut self, ctx: CommandContext<'_>, payload: &str) -> Result {
        ...
        if client.account().is_some() {
            ...
            ctx.rb
                .reply(rpl::ERR_SASLALREADY, 2 + lines::SASL_ALREADY.len(), |msg| {
        ...
        if payload == "*" && client.auth_id().is_some() {
            ctx.rb
                .reply(rpl::ERR_SASLABORTED, 2 + lines::SASL_ABORTED.len(), |msg| {
...
impl super::StateInner {
    pub fn cmd_setname(&mut self, ctx: CommandContext<'_>, real: &str) -> Result {
        ...
        real_response
            ...
            .param(real);
        ctx.rb
            .message(client.full_name(), Command::SetName, 0, |msg| {
                msg.param(real);
            });
